...
def getSpider(keywords, type_id):
    ...
    for index, x in enumerate(xpath_list, 1):
        ...
        img_path = os.path.abspath(os.path.join(path, "static/store_static/img/%s" % imgName))

        goods = Goods()
        goods.name = str(index).zfill(2) + keywords
        goods.store = 1
        goods.price = random.randint(20, 100)
        goods.safe_date = "%s-%s-%s" % (random.randint(1000, 9999), random.randint(1, 12), random.randint(1, 28))
        try:
            urlretrieve(img, img_path)
        except:
            continue
        else:
            goods.picture = "/store_static/img/" + imgName
        goods.number = random.randint(100, 1000)
        goods.desctiption = name
        goods.state = 1
